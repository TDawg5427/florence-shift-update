...
const DRAFT_KEY = 'florenceShiftDraft.v133';
...
// Use the actual current day unless an unfinished shift is restored later.
$('day').value = currentDayName();
...
function collectCurrentShift(){
  return {
    ...
    cash: entries('cashRows'),
    square: entries('squareRows'),
...
function restoreCurrentShift(){
  ...
  if(draft.day) $('day').value = draft.day;
  ...
  (draft.cash || []).forEach(x => addFromTemplate('cashTemplate','cashRows',x));
  (draft.square || []).forEach(x => addFromTemplate('squareTemplate','squareRows',x));
  ...
  document.querySelectorAll('.deepClean').forEach(x => {
    x.checked = deepCleans.has(x.value);
  });
...
// Restore an unfinished shift if the page/app reloads unexpectedly.
restoreCurrentShift();
generate({save:false});
